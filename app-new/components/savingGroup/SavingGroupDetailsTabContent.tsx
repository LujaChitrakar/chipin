...
import {
  useAddSavingGroupPayment,
  useDeleteExpense,
  useGetSavingGroupById,
  useUpdateExpense,
...
const SavingGroupTabContent = ({ activeTab }: { activeTab: string }) => {
      ...
      },
      {
        onSuccess: (response) => {
          ...
          addPayment(
            ...
            {
              onSuccess: () => {
                ToastAndroid.showWithGravity("Payment settled successfully.", ToastAndroid.SHORT, ToastAndroid.TOP + 10);
                setSettlingMemberId(member?._id);
  ...
  const { mutate: addPayment, isPending: addingPayment } = useAddSavingGroupPayment();
  ...
  const allBalances =
    (savingGroupData?.data?.balances &&
      savingGroupData?.data?.balances[myProfile?.data?._id || '--']) ||
    {};

  const expenses = savingGroupData?.data?.expenses || [];
  const isAdmin = savingGroupData?.data?.member_admins.includes(myProfile?.data?._id);
  ...
  const members = savingGroupData?.data?.members || [];
...
export default SavingGroupTabContent;
